...
const array = [6, 12, 'miami', 1, true, 'barca', '200', 'lisboa', 8, 10];
...
function averageNumbers(oneArray) {
  if (oneArray.length === 0) {
    return null;
  } else if (oneArray.length === 1) {
    return oneArray[0];
  } else {
    let average = sum(oneArray) / oneArray.length;
    return average;
  }
}
...
function averageWordLength(wordsArr) {
  if (wordsArr.length === 0) {
    ...
    let avgArray = sum(wordsArr) / wordsArr.length;
    return avgArray;
  }
}
...
function doesWordExist(wordArray, searchWord) {
  if (wordArray.length === 0) {
    ...
    return wordArray.includes(searchWord);
  }
}
